...
const start2 = new Song("so_lo", '  z x c v b n m ');
    ...
    document.addEventListener('keydown', e => {
        if (boolIsTurnOn == 1) {
            ...
            if (this.value[i] === e.key) {
                countTrue++;
                if (this.value.length / 2 - 2 == countTrue) {
                ...
                if (this.value[i] != e.key && this.value[i] != undefined&&countTrue!=0) {
...
function playNote(ch) {
    ...
    if (index < thisValue.length) {
        if (thisValue[index] != '|' || thisValue[index] != ' ') {
        ...
        setTimeout('playNote(fast)', ch); // Delay of fast milliseconds (0.3 seconds)
    }
    else
        index = 0
}
